...
if 'equipos' not in st.session_state:
    st.session_state.equipos = {
        ...
        "Monaco": ["Chesnaught", "Probopass", "Bisharp", "Tangrowth", "Orbeetle", "Perrserker-2", "Togetic", "Gurdurr", "Meowstic M", "Ribombee", "Sneasler-2", "Majecki"],
...
def simular_partido(local, visitante, comp, modo_prueba=False):
    ...
    for g, eq, plant in [(gl, local, p_l), (gv, visitante, p_v)]:
        for _ in range(g):
            ...
            # Solo sumar si NO es modo prueba
            if not modo_prueba:
...
if menu == "🏆 Competiciones":
    ...
    with t1:
        ...
        if modo_prueba:
            st.warning("⚠️ Los resultados actuales no se guardarán en las tablas.")
        
        txt = st.text_area("Formato: Local - Visitante")
        if st.button("Simular"):
            partidos_input = [l.strip() for l in txt.split("\n") if "-" in l]
            resultados = []
            for p in partidos_input:
                res = simular_partido(p.split("-")[0].strip(), p.split("-")[1].strip(), comp_activa, modo_prueba)
                resultados.append(res)
                # Solo guardar en el historial si NO es modo prueba
                if not modo_prueba:
                    st.session_state.data[comp_activa]["Historial"].append(res)
            
            # Mostrar los resultados del momento (sean prueba o no)
            for r in resultados:
                st.write(f"### {r['l']} {r['gl']} - {r['gv']} {r['v']} | MVP: {r['mvp']}")
                with st.expander("Goles"):
                    for e in r['evs']: st.write(f"{e['min']}' ⚽ {e['autor']} (Asist: {e['asist']})")

        # Mostrar historial oficial (solo los que no fueron prueba)
        st.markdown("---")
        st.subheader("📜 Historial Oficial del Torneo")
        for r in reversed(st.session_state.data[comp_activa]["Historial"]):
            st.caption(f"{r['l']} {r['gl']} - {r['gv']} {r['v']} (MVP: {r['mvp']})")
